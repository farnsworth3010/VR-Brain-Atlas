...
namespace Dummiesman
{
    public static class OBJLoaderHelper
    {
        ...
        {
            // Try several common shader names to support Built-in, URP, and other pipelines.
            ...
            if (shader == null)
            {
                Debug.LogWarning("OBJLoaderHelper: No suitable shader found. Creating material with fallback settings.");
                // As a last resort try to create a material without a shader reference to avoid ArgumentNullException
                Shader defaultShader = Shader.Find("Standard");
                if (defaultShader != null)
                {
                    return new Material(defaultShader);
                }
                // If still null, create a material via the parameterless constructor (available in newer Unity versions)
                try
                {
                ...
                {
                    // Last fallback: create material using built-in constructor may throw on some Unity versions; return null-safe material
